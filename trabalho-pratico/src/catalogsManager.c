...
void sortCatalogs(Catalogs *catalogs) {
    sortUsersByName(catalogs->usersCatalog);
...
void catalogs_compute_Q1_reservation (char *id, Catalogs* catalogs, QueryResult* result) {
    int ppn; //preço por noite
    int nnights; //número de noites
    char* hotel_id = reservation_catalog_compute_Q1(id,&ppn,&nnights,catalogs->reservationsCatalog,result);
    if (hotel_id == NULL) return;
    int cityTax = hotel_catalog_compute_Q1(hotel_id,catalogs->hotelsCatalog,result); //taxa turística
...
void catalogs_compute_Q4(char* id, Catalogs* catalogs, QueryResult* result){
    ...
    if (hotel==NULL) return; //se o id não existir
       ...
       for(i=0;i<listSize; i++){
            ...
            if (rating>=1){ //se o utilizador tiver dado uma classificação
            ...
            else{ //se o utilizador não tiver dado uma classificação
...
Result * catalogs_compute_Q10(int year, int month, int day, Catalogs* catalogs){
    ...
    OrdList* list_passengers = getFlightsDataQ10(year, month, day, catalogs->flightsCatalog, &flights, &passengers, &unique_passengers); //calcula o número de voos e passageiros e a list_passengers
    unique_passengers = count_unique_passengers(catalogs->usersCatalog,list_passengers); //calcula o número de passageiros únicos nessa data
    destroyOrdList(list_passengers,free);
    int reservations = getReservationsQ10(year, month, day, catalogs->reservationsCatalog); //número de reservas nessa data
    if (new_users==0 && flights==0 && reservations==0) return NULL; //se não existirem informações nessa data
    else{
...
//para otimização da query 10 - calcula o primeiro ano em que existem registos de criação de conta, voo, ou reserva
...
//para otimização da query 10 - calcula o último ano em que existem registos de criação de conta, voo, ou reserva
